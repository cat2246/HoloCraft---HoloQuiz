...
- `Chat Trigger` lets you create log-line triggers with a `Trigger Phase`, optional `Micro`, optional MP3/WAV sound, cooldown, and per-trigger typing interval. Each trigger must have a Micro, a sound, or both. Use `Browse...` to select a sound; its path is saved in `config.json`. The trigger `Dry-run` checkbox is separate from the main HoloQuiz dry-run and suppresses keyboard macros, while configured sounds still play. For example, trigger on `Good Morning!` and use `tGood Morning{{Enter}}` to open chat, type the message, and press Enter. Set `Typing interval` to `0.1` if you want typed characters or repeated macro tokens to wait 0.1 seconds before the next input. Tokens use double braces, such as `{{Enter}}`, `{{Escape}}`, `{{Ctrl+V}}`, `{{LButton}}`, `{{RButton}}`, and `{{MButton}}`.
